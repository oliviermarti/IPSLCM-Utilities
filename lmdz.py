...
lon_per = xr.DataArray (360.0)     ; lon_per.name='lon_per' ; lon_per.attrs.update  ({'units':"degrees_east"  , 'long_name':"Longitude range" })
...
RDAY    = RJJHH * RHHMM * RMMSS                 ; RDAY.name   = 'RDAY'   ; RDAY.attrs.update   ({'units':"second", 'long_name':"Day length"})
RSIYEA  = 365.25 * RDAY * 2. * np.pi / 6.283076 ; RSIYEA.name = 'RSIYEA' ; RSIYEA.attrs.update ({'units':"second", 'long_name':"Sideral year length"})
RSIDAY  = RDAY / (1. + RDAY / RSIYEA)           ; RSIDAY.name = 'RSIDAY' ; RSIDAY.attrs.update ({'units':"second", 'long_name':"Sideral day length"})
...
def add_cyclic (ptab, x, y, axis=-1, cyclic=lon_per, precision=0.0001, Debug=False) :
    ...
    ztab, xx, yy = cutil.add_cyclic (data   = ptab,
                                     ...
                                     y      = yy1,
                                     axis   = axis,
                                     cyclic = libIGCM_utils.xr_dequantify(lon_per).item(),
                                     precision=0.0001)
