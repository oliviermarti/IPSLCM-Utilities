...
Utilities to process and plot NEMO ORCA fields,
...
NPERIO_LITERAL = Literal[0, 1, 4, 5, 6,]
NFTYPE_LITERAL = Literal['T', 'F']
CDTYPE_LITERAL = Literal['T', 'U', 'V', 'F', 'W']
AXIS_LITERAL   = Literal['x', 'y', 'z', 't', 'b', 'c', 'bnds']
...
LONNAME:list[str]=['nav_lon', 'nav_lon_T', 'nav_lon_U', 'nav_lon_V',
                   'nav_lon_F', 'nav_lon_W',
                   'nav_lon_grid_T', 'nav_lon_grid_U', 'nav_lon_grid_V',
                   'nav_lon_grid_F', 'nav_lon_grid_W']
LATNAME:list[str]=['nav_lat', 'nav_lat_T', 'nav_lat_U', 'nav_lat_V', 'nav_lat_F', 'nav_lat_W',
                   'nav_lat_grid_T', 'nav_lat_grid_U', 'nav_lat_grid_V',
                   'nav_lat_grid_F', 'nav_lat_grid_W']
...
YXNAME:list[str] = ['yx', 'xy', 'y_c_x_c', 'y_c_x_f', 'y_f_x_c', 'y_f_x_f',
                    'yx_grid_T', 'yx_grid_U', 'yx_grid_V', 'yx_grid_F', 'oce_grid_size']

TNAME:list[str] = [ 't', 'T', 'tt', 'TT', 'time', 'time_counter', 'time_centered',
                    'time', 'TIME', 'TIME_COUNTER', 'TIME_CENTERED', ]
BNAME:list[str] = [ 'b', 'bnd', 'bnds', 'bound', 'bounds', 'vertex', 'nvertex',
                   'two', 'two1', 'two2', 'four', 'axis_nbounds']
BNAME.extend ( [f'nvertex_grid_{g}' for g in ['T', 'U', 'V', 'W', 'F'] ] )
...
Ttab:xr.DataArray = xr.DataArray (np.linspace (-2, 40, 100), dims=('Temperature',), coords=(np.linspace (-2, 40, 100),), attrs={'unit':'degrees_celcius', 'long_name':'Temperature'})
Stab:xr.DataArray = xr.DataArray (np.linspace ( 0, 40, 100), dims=('Salinity'   ,), coords=(np.linspace ( 0, 40, 100),), attrs={'unit':'PSU'            , 'long_name':'Salinity'   })
...
Regions:dict = \
    { 'region' : {
      ...
      'eORCA1': {
        ...
        'Rockall'          : {'idyx': {'x':slice(265,290), 'y':slice(263,283)}},
      ...
      'ORCA2': {
        ...
        'Rockall'          : {'idyx': {'x':slice(130,143), 'y':slice(111,121)}},
        'MedWest'          : {'idyx': {'x':slice(141,153), 'y':slice(100,108)}},
        'MedEast'          : {'idyx': {'x':slice(153,179), 'y':slice( 95,105)}},
        'Wedell'           : {'idyx': {'x':slice(112,149), 'y':slice( 52, 25)}},
        'Davis'            : {'idyx': {'x':slice(  1, 10), 'y':slice( 10, 25)}},
        'CircumPolar'      : {'idyx': {'x':slice(  2,180), 'y':slice( 52, 25)}},
      }
   }
...
Straits:dict = \
    { 'strait' :  {
        'drake_passage'           : {'Strait':'Drake Passage', },
        'barents_opening'         : {'Strait':'Barents opening'         },
        'bering_strait'           : {'Strait':'Bering Strait'           },
        'canadian_archipelago'    : {'Strait':'Canadian Archipelago'    },
        'denmark_strait'          : {'Strait':'Denmark Strait'          },
        'english_channel'         : {'Strait':'English Channel'         },
        'equatorial_undercurrent' : {'Strait':'Pacific Equatorial Undercurrent'},
        'florida_bahamas'         : {'Strait':'Florida-Bahamas'         },
        'fram_strait'             : {'Strait':'Fram Strait'             },
        'norwegian_basin'         : {'Strait':'Island-Norge'            },
        'indonesian_throughflow'  : {'Strait':'Indonesian throughflow'  },
        'mozambic_channel'        : {'Strait':'Mozambic channel'        },
        'gibraltar_strait'        : {'Strait':'Gibraltar strait'        },
        'sumatra_borneo'          : {'Strait':'Sumatra-Borneo'          },
        },
      'eORCA1': {
          'drake_passage'           : {'idyx': {'y':slice( 79,106), 'x':slice(221,222) },},
          'bering_strait'           : {'idyx': {'y':slice(285,286), 'x':slice(113,118) },},
          'denmark_strait'          : {'idyx': {'y':slice(282,283), 'x':slice(250,267) },},
          'equatorial_undercurrent' : {'idyx': {'y':slice(179,193), 'x':slice(132,133) },},
          'florida_bahamas'         : {'idyx': {'y':slice(224,230), 'x':slice(208,209) },},
          'fram_strait'             : {'idyx': {'y':slice(312,313), 'x':slice(268,279) },},
          'norwegian_basin'         : {'idyx': {'y':slice(282,283), 'x':slice(271,291) },},
          'canadian_archipelago'    : {'idyx': {'y':slice(320,321), 'x':slice(228,233) },},
          'indonesian_throughflow'  : {'idyx': {'y':slice(164,165), 'x':slice( 43, 70) },},
          'mozambic_channel'        : {'idyx': {'y':slice(154,155), 'x':slice(326,333) },},
          'gibraltar_strait'        : {'idyx': {'y':slice(239,242), 'x':slice(283,284) },},
          'sumatra_borneo'          : {'idyx': {'y':slice(187,188), 'x':slice( 32, 39) },},
                 },
      'ORCA2' : {
          ...
          'norwegian_basin'         : {'idyx': {'y':slice(122,123), 'x':slice(134,145) },},
          'indonesian_throughflow'  : {'idyx': {'y':slice( 62, 63), 'x':slice( 24, 32) },},
          'mozambic_channel'        : {'idyx': {'y':slice( 57, 58), 'x':slice(162,165) },},
          'gibraltar_strait'        : {'idyx': {'y':slice(100,103), 'x':slice(140,141) },},
          'sumatra_borneo'          : {'idyx': {'y':slice( 76, 77), 'x':slice( 13, 18) },},
                 },
     }

Straits['eORCA1.2']   = Straits['eORCA1']
Straits['eORCA1.4']   = Straits['eORCA1']
Straits['eORCA1.4.2'] = Straits['eORCA1']

Straits['ORCA2.3']    = Straits['ORCA2']
Straits['ORCA2.4']    = Straits['ORCA2']
...
class Domain :
    ...
    @validate_types
    def replace (self:Self, dico:Optional[Dict[str,Any]]=None, action:str|None=None, fatal:bool=True, Debug:bool=False, **kwargs:Any) -> None :
        '''Use a dictionnary to update values
        if action is set, add/del halo or cyclic
        '''
        if dico :
    ...
    def __init__ (self:Self, ptab:xr.DataArray|None=None, cfg_name:str|None=None, CFG_name:str|None=None,
        ...
        Tries to infer all missing values from known ones
        '''
        ...
        if ptab is not None :
            ...
            az, _ = find_axis (ptab, 'z')

            if zOPTIONS['Debug'] or Debug :
                print (f'{ax=}, {ay=}, {az=}')

            zjpi = ptab.sizes[ax] if ax is not None else None
            zjpj = ptab.sizes[ay] if ay is not None else None
            zjpk = ptab.sizes[az] if az is not None else None
    ...
    def edit (self:Self, action:str|None=None, Debug:bool=False, stop_on_check:bool=False) -> None :
        ...
        '''
        push_stack ('edit domain')
        if action in [ 'add_halo', 'add', 'ext', 'extend'] :
            self.add_halo   (Debug=Debug, stop_on_check=stop_on_check)
        if action in [ 'del_halo', 'del', 'rm', 'remove'] :
            self.del_halo   (Debug=Debug, stop_on_check=stop_on_check)
        if action in [ 'add_cyclic', 'cyclic' ]:
            self.add_cyclic (Debug=Debug, stop_on_check=stop_on_check)
        if action in [ 'del_cyclic', ] :
            self.del_cyclic (Debug=Debug, stop_on_check=stop_on_check)
        pop_stack ('edit_domain')

    @validate_types
    def add_halo (self:Self, Debug:bool=False, stop_on_check:bool=False) :
        '''
        Add halo to the domain
        '''
        push_stack ('add_halo')
        if self.Halo :
            if stop_on_check :
    ...
    @validate_types
    def del_halo (self:Self, Debug:bool=False, stop_on_check:bool=False) -> None :
        '''
        Delete halo from the domain
        '''
        push_stack ('del_halo')
        if not self.Halo :
            if stop_on_check :
    ...
    @validate_types
    def add_cyclic (self:Self, Debug:bool=False, stop_on_check:bool=False) -> None :
        '''
        Add cyclic to the domain
        '''
    ...
    @validate_types
    def del_cyclic (self:Self, Debug:bool=False, stop_on_check:bool=False) :
        '''
        Delete cyclic from the domain
        '''
        pop_stack ('del_cyclic')
        if not self.Halo :
            if stop_on_check :
                raise RuntimeError ( f'nemo.Domain.del_cyclic: can not delete Cyclic to domain with Cyclic=False \n domain={self}')
            elif OPTIONS['Debug'] or Debug :
                print           ( f'nemo.Domain.del_cyclic: can not delete Cyclic to domain with Cyclic=False \n domain={self}')

        elif self.Cyclic :
            if stop_on_check :
                raise RuntimeError ( f'nemo.Domain.del_cyclic: can not delete Halo to domain with Cyclic=True \n domain={self}')
            elif OPTIONS['Debug'] or Debug :
...
class GridMask :
    ...
    '''
    @validate_types
    def area (self:Self, cd_type='T', Debug=False ) :
        zarea = None
        match cd_type :
            case ( 'T' | 't' ) :
                if OPTIONS['Debug'] or Debug :
                    print ( "case T" )
                if self.e1t is not None and self.e2t is not None :
                    if OPTIONS['Debug'] or Debug :
                        print ( "Computing" )
                    zarea = self.e1t*self.e2t
        return zarea

    def vol (self:Self, cd_type='T' ) :
        zvol = None
        match cd_type :
            case ( 'T' | 't' ) :
                if self.e1t is not None and self.e2t is not None and self.e3t is not None :
                    zarea = self.e1t*self.e2t*self.e3t
        return zvol
    
    def __init__ (self:Self, mm:libIGCM.sys.Config, domain:Domain,
        ...
        f_d2 = os.path.join (mm.R_IN, 'OCE', 'NEMO', domain.CFG_name, 'GRIDS', f'{domain.CFG_name}_domain_cfg.nc'        ) # type: ignore
        ...
        if 'glamt' in d_g : # pyright: ignore[reportOperatorIssue]
            lon_T   = lbcu (d_g.glamt, cd_type='T', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lat_T   = lbcu (d_g.gphit, cd_type='T', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lon_U   = lbcu (d_g.glamu, cd_type='U', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lat_U   = lbcu (d_g.gphiu, cd_type='U', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lon_V   = lbcu (d_g.glamv, cd_type='V', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lat_V   = lbcu (d_g.gphiv, cd_type='V', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lon_F   = lbcu (d_g.glamf, cd_type='F', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lat_F   = lbcu (d_g.gphif, cd_type='F', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
        elif 'nav_lon_grid_T' in d_g : # pyright: ignore[reportOperatorIssue]
            lon_T   = lbcu (d_g.nav_lon_grid_T, cd_type='T', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lat_T   = lbcu (d_g.nav_lat_grid_T, cd_type='T', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lon_U   = lbcu (d_g.nav_lon_grid_U, cd_type='U', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lat_U   = lbcu (d_g.nav_lat_grid_V, cd_type='U', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lon_V   = lbcu (d_g.nav_lon_grid_V, cd_type='V', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]
            lat_V   = lbcu (d_g.nav_lat_grid_V, cd_type='V', domain=domain, btype='lbc', **kw_uni) # pyright: ignore[reportOptionalMemberAccess]

        lon_T = unify_dims (lon_T, **kw_uni, xgrid='T')
        lon_U = unify_dims (lon_U, **kw_uni, xgrid='U')
        lon_V = unify_dims (lon_V, **kw_uni, xgrid='V')
        lon_F = unify_dims (lon_F, **kw_uni, xgrid='F')
        lat_T = unify_dims (lat_T, **kw_uni, xgrid='T')
        lat_U = unify_dims (lat_U, **kw_uni, xgrid='U')
        lat_V = unify_dims (lat_V, **kw_uni, xgrid='V')
        lat_F = unify_dims (lat_F, **kw_uni, xgrid='F')

        lon_T   = fixed_lon (lon_T)
        lon_U   = fixed_lon (lon_U)
        lon_V   = fixed_lon (lon_V)
        lon_F   = fixed_lon (lon_F)

        #if lat_T is not None and lon_T is not None :
        #    lat_T, lont_T = fill_latlon (lat_T, lon_T)
        #if lat_U is not None and lon_U is not None :
        #    lat_U, lont_U = fill_latlon (lat_U, lon_U)
        #if lat_V is not None and lon_V is not None :
        #    lat_V, lont_V = fill_latlon (lat_V, lon_V)
        #if lat_F is not None and lon_F is not None :
        #    lat_F, lont_F = fill_latlon (lat_F, lon_F)
        ...
        maskdraw_V = lbc_plot (mask_V, cd_type='V', domain=domain)
        maskdraw_F = lbc_plot (mask_F, cd_type='F', domain=domain)
        maskdraw_W = lbc_plot (mask_W, cd_type='T', domain=domain)

        maskutil_T = lbc_mask (mask_T, cd_type='T', domain=domain)
        maskutil_U = lbc_mask (mask_U, cd_type='U', domain=domain)
        ...
        gsintT, gcosT, gsinU, gcosU, gsinV, gcosV, gsinF, gcosF = None, None, None, None, None, None, None, None,
        if lat_T is not None and lon_T is not None : 
            gsinT, gcosT = angle (lon_T, lat_T , domain=domain, cd_type='T')
        if lat_U is not None and lon_U is not None : 
            gsinU, gcosU = angle (lon_U, lat_U , domain=domain, cd_type='U')
        if lat_V is not None and lon_V is not None : 
            gsinV, gcosV = angle (lon_V, lat_V , domain=domain, cd_type='V')
        if lat_F is not None and lon_F is not None : 
            gsinF, gcosF = angle (lon_F, lat_F , domain=domain, cd_type='F')

        atlmsk, atlmsk_nomed, pacmsk, ipcmsk, indmsk = None, None, None, None, None
        ...
        e3t_0  = d_g.e3t_0  if 'e3t_0'  in d_g.variables else None # pyright: ignore[reportOptionalMemberAccess]
        e3w_0  = d_g.e3w_0  if 'e3w_0'  in d_g.variables else None # pyright: ignore[reportOptionalMemberAccess]
        e3t_ps = d_g.e3t_ps if 'e3t_ps' in d_g.variables else None # pyright: ignore[reportOptionalMemberAccess]
        e3w_ps = d_g.e3w_ps if 'e3w_ps' in d_g.variables else None # pyright: ignore[reportOptionalMemberAccess]
        ...
        if d_e3 is not None and 'e3t' in d_e3.variables :
            ...
            if e3t_0 is not None :
                ...
                e3t = None
        if d_e3 is not None and 'e3w' in d_e3.variables :
            e3w = d_e3.e3w
        else :
        ...
        z_c, z_f, ze3t, ze3w = None, None, None, None
        if 'orca2' in domain.cfg_name :
            zz_c, zz_f, ze3t, ze3w = domzgr.zgr_z (config='orca2')
        if 'eorca1' in domain.cfg_name :
            zz_c, zz_f, ze3t, ze3w = domzgr.zgr_z (config='eorca1')

        if z_c is None and zz_c is not None :
            z_c = unify_dims (zz_c, **kw_uni, xgrid='T')
        if z_f is None and zz_f is not None :
            z_f = unify_dims (zz_f, **kw_uni, xgrid='W')
        if e3t is None and ze3t is not None :
            e3t = unify_dims (ze3t, **kw_uni, xgrid='T')
        if e3w is None and ze3w is not None :
            e3w = unify_dims (ze3w, **kw_uni, xgrid='W')

        #e3u = t2u (e3t) if e3t is not None else None
        #e3v = t2v (e3t) if e3t is not None else None
        #e3f = t2f (e3t) if e3t is not None else None
        e3u = unify_dims (e3t, **kw_uni, xgrid='U')
        e3v = unify_dims (e3t, **kw_uni, xgrid='V')
        e3f = unify_dims (e3t, **kw_uni, xgrid='F')

        #bt = area_t*e3t*mask_3T if area_T is not None and e3t is not None and mask_3T is not None else None

        # Builds 1d bounds of z_c for xgcm
        az_c, ik_c = find_axis (z_c, 'z')
        az_f, ik_f = find_axis (z_f, 'z')
        
        lshape = list (z_c.shape)
        lshape [ik_f] = lshape[ik_f]+1
        ldims = list (z_c.dims)
        ndim  = f'{ldims[ik_f]}_bnds1d' # type: ignore
        ldims [ik_f] = f'{ldims[ik_f]}_bnds1d'
        
        z_c_bnds1d = xr.DataArray (np.empty (lshape), dims=ldims)
        z_c_bnds1d [{ndim:slice(0,-1)}] = z_f.values
        z_c_bnds1d [{ndim:-1}]          = z_f.isel({az_f:-1}).values
        ...
        # sea_poly  , sea_poly_shp   = build_feat (libIGCM.sys.Dap2Thredds(os.path.join (mm.DB, 'extras', f'{domain.cfg_name}_sea.json')      , mm),
        #                                                facecolor='none', edgecolor='grey') # type: ignore
        # grid_poly , grid_poly_shp  = build_feat (libIGCM.sys.Dap2Thredds(os.path.join (mm.DB, 'extras', f'{domain.cfg_name}_gridboxes.json'), mm),
        #                                                facecolor='none', edgecolor='grey') # type: ignore
        # grid_line , grid_line_shp  = build_feat (libIGCM.sys.Dap2Thredds(os.path.join (mm.DB, 'extras', f'{domain.cfg_name}_gridlines.json'), mm),
        #                                               facecolor='k', edgecolor='none') # type: ignore

        if mm.DB is not None :
            f_coast     = os.path.join (mm.DB, 'extras', f'{domain.cfg_name}_coast.json'    )
            f_land      = os.path.join (mm.DB, 'extras', f'{domain.cfg_name}_land.json'     )
            f_sea       = os.path.join (mm.DB, 'extras', f'{domain.cfg_name}_sea.json'      )
            f_gridboxes = os.path.join (mm.DB, 'extras', f'{domain.cfg_name}_gridboxes.json')
            f_gridlines = os.path.join (mm.DB, 'extras', f'{domain.cfg_name}_gridlines.json')
        else :
        ...
        # Defines the class components

        self.cfg_name = domain.cfg_name
        self.CFG_name = domain.CFG_name
        self.cd_cfg   = domain.cd_cfg
        self.kk_cfg   = domain.kk_cfg
        self.jpk      = domain.jpk
        self.jpj      = domain.jpj
        self.jpi      = domain.jpi
        self.Iperio   = domain.Iperio
        self.Jperio   = domain.Jperio
        self.NFold    = domain.NFold
        self.NFtype   = domain.NFtype
        self.nperio   = domain.nperio
        self.aperio   = domain.aperio
        ...
        self.sea_poly   = sea_poly
        self.grid_poly  = grid_poly
        self.grid_line  = grid_line

        self.coast_poly_shp = coast_poly_shp
        self.land_poly_shp  = land_poly_shp
        self.sea_poly_shp   = sea_poly_shp
        self.grid_poly_shp  = grid_poly_shp
        self.grid_line_shp  = grid_line_shp

        self.maskdraw_T = maskdraw_T
        self.maskdraw_U = maskdraw_U
        self.maskdraw_V = maskdraw_V
        self.maskdraw_F = maskdraw_F
        self.maskdraw_W = maskdraw_W

        self.maskutil_T = maskutil_T
        self.maskutil_U = maskutil_U
        self.maskutil_V = maskutil_V
        self.maskutil_F = maskutil_F
        self.maskutil_W = maskutil_W

@validate_types
def essai ( a:int|float, b:Domain) : # pylint: disable=missing-function-docstring
    return a, b.Halo
...
def add_halo (domain:Domain, Debug:bool=False, stop_on_check:bool=False) -> Domain :
    ...
    push_stack ('add_halo')
    zdom = domain.copy()
    zdom.add_halo (Debug=Debug, stop_on_check=stop_on_check)
    pop_stack ('del_halo')
    return zdom

@validate_types
def del_halo (domain, Debug:bool=False, stop_on_check:bool=False) -> Domain :
    '''
    Delete halo from the domain
    '''
    push_stack ('add_halo')
    zdom = domain.copy()
    zdom.del_halo (Debug=Debug, stop_on_check=stop_on_check)
    pop_stack ('del_halo')
    return zdom

@validate_types
def add_cyclic (domain, Debug:bool=False, stop_on_check=False) -> Domain :
    '''
    Add cyclic to the domain
    '''
    push_stack ('add_cyclic')
    zdom = domain.copy()
    zdom.add_cyclic (Debug=Debug, stop_on_check=stop_on_check)
    pop_stack ('add_cyclic')
    return zdom

@validate_types
def del_cyclic (domain, Debug:bool=False, stop_on_check:bool=False) -> Domain :
    '''
    Delete cyclic from the domain
    '''
    push_stack ('del_cyclic')
    zdom = domain.copy()
    zdom.del_cyclic (Debug=Debug, stop_on_check=stop_on_check)
    pop_stack ('del_cyclic')
    return zdom
...
@validate_types
def find_axis (ptab:xr.DataArray|xr.Dataset, axis:str|Literal['x', 'y', 'z', 't', 'yx', 'b']='z',
            ...
            if 'coords' in dir(ptab) :
                if 'axis' in ptab.coords[dim].attrs.keys() :
                    ...
                    if l_axis in ax_name and l_axis == str('X'):
                        if OPTIONS['Debug'] or Debug:
                            print ( 'Rule 1 : xaxis found by attribute : ',
                                   f'{ax=} {l_axis=} {axis=} : {ax_name=} {l_axis=} {ii=} {dim=}' )
                        ix, ax = ii, str(dim)
                    if l_axis in ax_name and l_axis == str('Y'):
                        if OPTIONS['Debug'] or Debug:
                            print ( 'Rule 1 : yaxis found by attribute : ',
                                   f'{ax=} {l_axis=} {axis=} : {ax_name=} {l_axis=} {ii=} {dim=}' )
                        ix, ax = ii, str(dim)
                    if l_axis in ax_name and l_axis == str('Z'):
                        if OPTIONS['Debug'] or Debug:
                            print ( 'Rule 1 : zaxis found by attribute : ',
                                f'{ax=} {l_axis=} {axis=} : {ax_name=} {l_axis=} {ii=} {dim=}' )
                        ix, ax = ii, str(dim)
                    if l_axis in ax_name and l_axis == str('T') :
                        if OPTIONS['Debug'] or Debug:
                            print ( 'Rule 1 : taxis found by attribute : ',
                                f'{ax=} {l_axis=} {axis=} : {ax_name=} {l_axis=} {ii=} {dim=}' )
                        ix, ax = ii, str(dim)
...
def build_bounds2d (glont:xr.DataArray|None=None, glatt:xr.DataArray|None=None,
    ...
    if glonf is not None and glatf is not None :
        ...
        bounds_lont [nver_list[0]] = glonf.roll ({ayf: 1,axf: 1}) # type: ignore
        ...
        if glont is not None :
            ...
            bounds_lont.attrs.update (glont.attrs) # type: ignore[reportAttributeAccessIssue]
        else :
            bounds_lont = clo_lon (bounds_lont, glonf)
            bounds_lont.attrs.update (glonf.attrs) # type: ignore[reportAttributeAccessIssue]
        if glatt is not None :
    ...
    if glonv is not None and glatv is not None :
        ...
        if glonu is not None :
            ...
            bounds_lonu.attrs.update (glonu.attrs) # type: ignore[reportAttributeAccessIssue]
        else :
            bounds_lonu = clo_lon (bounds_lonu, glonv)
            bounds_lonu.attrs.update (glonv.attrs) # type: ignore[reportAttributeAccessIssue]
        if glatu is not None :
    ...
    if glonu is not None and glatu is not None :
        ...
        if glonv is not None :
            ...
            bounds_lonv.attrs.update (glonv.attrs) # type: ignore[reportAttributeAccessIssue]
        else :
            bounds_lonv = clo_lon (bounds_lonv, glonu)
            bounds_lonv.attrs.update (glonu.attrs) # type: ignore[reportAttributeAccessIssue]
        if glatv is not None :
    ...
    if rpoint is None :
        ...
        if rpoint in ['u', 'U'] :
            push_stack ( 'build_bounds2d : U point' )
            return bounds_lonu, bounds_latu
        if rpoint in ['v', 'V'] :
            push_stack ( 'build_bounds2d : V point' )
            return bounds_lonv, bounds_latv
        if rpoint in ['f', 'F'] :
            push_stack ( 'build_bounds2d : F point' )
...
def close_bounds (blon:xr.DataArray, blat:xr.DataArray|None=None) -> tuple[xr.DataArray, xr.DataArray]|xr.DataArray :
    ...
    '''
    push_stack ('close_bounds')
    ab, _ = find_axis ( blon, 'bnds')
    blon = xr.concat ( [blon, blon[{ab:-1}]], dim=ab )
    if blat is not None :
        ab, _ = find_axis ( blon, 'bnds')
        blat = xr.concat ( [blat, blat[{ab:-1}]], dim=ab )
        pop_stack ('close_bounds : lon, lat' )
        return blon, blat
    else :
        pop_stack ('close_bounds : lon' )
        return blon

@validate_types
def build_bounds1d (b2d:xr.DataArray, Debug:bool=False) -> xr.DataArray :
    '''
    Build 1D bounds variable at the XGCM format from 2D bounds variable
    '''
...
def fixed_lon (plon:xr.DataArray|None, center_lon:float=0.0, Debug:bool=False) -> xr.DataArray|None :
    ...
    if plon is not None :
        ...
        if plon.sizes [ax] == 1442 :
            f_lon [..., -2, :] = f_lon [..., -3, :]
        if plon.sizes [ax] == 1440 :
            f_lon [..., -1, :] = f_lon [..., -2, :]
    ...
    if isinstance (ptab, xr.Dataset) :
        for varname in ptab.variables :
            if OPTIONS['Debug'] or Debug :
                print (f'{varname=}')
            pvar = ptab[varname]
            ax, _ = find_axis (pvar, 'x')
            ay, _ = find_axis (pvar, 'y')
            ...
            if ax in XNAME and ay in YNAME :
                ...
                ptab[varname] = lbcu (ptab[varname], x=x, y=y, z=z, t=t, use_xgcm=use_xgcm,
                                    cd_type=lcd_type,
                                    psgn=lpsign, nemo_4U_bug=nemo_4U_bug,
                                    Iperio=Iperio, Jperio=Jperio, NFold=NFold, NFtype=NFtype,
                                    Halo=Halo, Cyclic=Cyclic, aperio=aperio, nperio=nperio,
                                    domain=domain, Debug=Debug, btype=btype)
    elif isinstance (ptab, xr.DataArray) :
        ...
        if btype == 'plot' :
            ptab = lbc_plot (ptab, cd_type=cd_type, psgn=psgn, Iperio=Iperio, Jperio=Jperio,
                             NFold=NFold, NFtype=NFtype, Halo=Halo, Cyclic=Cyclic, aperio=aperio,
...
def fill_empty (ptab:xr.DataArray, sval:float=np.nan, transpose:bool=False) -> xr.DataArray :
    ...
    if SimpleImputer is not None :
        ...
        print ( f'{ptab.sizes=} {sval=} {transpose=}' )
        pop_stack ( 'fill_empty [void version]' )
        return ptab

@validate_types
def fill_latlon (plat:xr.DataArray, plon:xr.DataArray, sval:float=0.,
    ...
    if SimpleImputer is not None :
        ...
        zlon = imp.transform (plon.values)
        if OPTIONS['Debug'] or Debug :
            print (f'{type(zlon)=}')
        ...
        zlat = xr.DataArray (zlat, coords=plat.coords, dims=plat.dims, attrs=plat.attrs)
        zlon = xr.DataArray (zlon, coords=plon.coords, dims=plon.dims, attrs=plon.attrs)
        ...
        print ( 'Can not call fill_empty' )
        print ( 'Call arguments where : ' )
        print ( f'{plat.sizes=} {sval=}' )

        pop_stack ( 'fill_latlon [void version]')
        return plat, plon

@validate_types
def fill_bounds_lonlat (pbounds_lon:xr.DataArray, pbounds_lat:xr.DataArray,
                        sval:int|float=-1) -> tuple[xr.DataArray, xr.DataArray] :
    '''
    ...
    if SimpleImputer is not None :
        ...
        print ( 'Can not call fill_empty' )
        print ( 'Call arguments where : ' )
        print ( f'{pbounds_lat.sizes=} {sval=}' )
...
def jeq (plat:xr.DataArray, Debug=False) -> int :
    ...
    if OPTIONS['Debug'] or Debug :
        print ( f'{ay=}, {jy=}, {plat.sizes=}' )

    jmin = plat.sizes[ay]//6 # Trick because latitude on Antartic may be at zero
    aa = np.argmin (np.abs (plat.isel({ay:slice(jmin,None)}).values), axis=jy)
    if OPTIONS['Debug'] or Debug :
        print ( f"{aa=}" )
        
    jj = int (np.nanmean (np.where (aa!=0, aa, np.nan))) + jmin
    jj = np.minimum ( jj, plat.sizes[ay]-1 )
...
def lon1d (plon:xr.DataArray, plat:xr.DataArray|None=None, Debug:bool=False, dim='x_c') -> xr.DataArray :
    ...
    ax, _ = find_axis (plon, 'x', Debug=Debug)
    ay, _ = find_axis (plon, 'y', Debug=Debug)
    jpj, jpi  = plon.sizes [ay], plon.sizes [ax] # type: ignore
    if plat is not None :
        je     = jeq (plat)
...
@validate_types
def latreg (plat:xr.DataArray, diff:float=0.1, Debug:bool=False) -> tuple[int, float] :
    '''
    ...
    if OPTIONS['Debug'] or Debug :
        print ( f'{plat.shape=} {plat.min().values=} {plat.max().values=}' )
    ...
    if diff is None :
        dy = np.mean (np.abs (plat - plat.roll ({ay:1}, roll_coords=False)))
        if OPTIONS['Debug'] or Debug :
            print ( f'latreg : {dy=}' )
        diff = dy/100.
    ...
    if ix and plat.sizes[ax] > 1 :
        ...
        jreg = np.minimum ( jreg, int(len(plat))-2 )
        lareg  = plat.isel({ay:jreg}).mean(dim=ax).item()
    else :
        jreg  = int (len (plat)-2)
        lareg = np.max (plat).item()
...
@validate_types
def lat1d (plat:xr.DataArray, dim='y_c', Debug:bool=False) -> xr.DataArray :
    '''
    ...
    ax, _ = find_axis (plat, 'x')
    ay, _ = find_axis (plat, 'y')
    jpj = plat.sizes[ay] # type: ignore

    dy = (plat.max() - plat.min())/plat.sizes[ay]
    je    = jeq (plat)
    lat_N = np.minimum (90.0, plat.max()+dy)

    if ax :
        lat_eq = plat[...,je,:].mean(dim=ax)
    else  :
        lat_eq = plat[...,je]

    jreg, lat_reg = latreg (plat, Debug=Debug)

    if ax :
        lat_ave = plat.mean (dim=ax)
    else  :
        lat_ave = plat
    ...
    if OPTIONS['Debug'] or Debug :
        print (f'{lat_N} {jreg=} {lat_reg=} {dy=} {yrange=}')
    ...
    # Et le sud ?
    js = lat_1d.argmin().item()  
    ys = np.maximum (-80, lat_1d[js+1].values.item()-5.)
...
def latlon1d (plat:xr.DataArray, plon:xr.DataArray, dims=('y_c', 'x_c'), Debug:bool=False) -> tuple[xr.DataArray, xr.DataArray] :
    ...
    zla = lat1d (plat, dim=dims[0], Debug=Debug)
    zlo = lon1d (plon, plat, dim=dims[1], Debug=Debug)
...
def beta (plat:xr.DataArray) -> xr.DataArray :
    ...
    zbeta = ROMEGA / RA * np.cos (RAD*plat)
    pop_stack ( 'beta' )
    return zbeta
...
def extend (ptab:xr.DataArray, blon:bool=False, jplus:int=25, jpi:int|None=None,
    ...
    if ptab.sizes[ax] == 1 :
        ...
        if ptab.sizes[ax] > jpi :
            ...
            tabex = xr.DataArray ( tabex, dims=ptab.dims, coords=new_coords )
...
@validate_types
def lbc_add (ptab:xr.DataArray|None, cd_type:CDTYPE_LITERAL|str|None=None, psgn:int|float=1,
             Iperio:bool|None=None, Jperio:bool|None=None, NFold:bool|None=None,
             NFtype:NFTYPE_LITERAL|None=None, Halo:bool|None=None,
             Cyclic:bool|None=None, aperio:int|float|None=None, nperio:int|None=None,
             domain:Domain|None=None, Debug:bool=False) -> xr.DataArray|None :
    '''
...
@validate_types
def lbc_add_cyclic (ptab:xr.DataArray|None, cd_type:CDTYPE_LITERAL|str|None=None, psgn:int|float=1,
            Iperio:bool|None=None, Jperio:bool|None=None, NFold:bool|None=None,
            NFtype:NFTYPE_LITERAL|str|None=None,
            Halo:bool|None=None, Cyclic:bool|None=None, aperio:int|float|None=None,
             nperio:int|None=None, domain:Domain|None=None, Debug:bool=False) -> xr.DataArray|None :
    '''
    ...
    if ptab is not None :
        ...
        if zdom.Iperio and zdom.NFold and not zdom.Halo and not zdom.Cyclic :
            tdom = zdom.copy()
            tdom.add_cyclic (stop_on_check=True)
...
@validate_types
def lbc_del (ptab:xr.DataArray|None, cd_type:CDTYPE_LITERAL|str='T', psgn:int|float=1,
             Iperio:bool|None=None, Jperio:bool|None=None,
             NFold:bool|None=None, NFtype:NFTYPE_LITERAL|None=None,
             Halo:bool|None=None, Cyclic:bool|None=None, aperio:int|float|None=None,
             nperio:int|None=None, domain:Domain|None=None, Debug=False) -> xr.DataArray|None :
    '''
    Handles NEMO domain changes between NEMO 4.0 to NEMO 4.2
...
@validate_types
def lbc_del_cyclic (ptab:xr.DataArray|None, cd_type:CDTYPE_LITERAL|str='T', psgn:int|float=1,
            Iperio:bool|None=None, Jperio:bool|None=None, NFold:bool|None=None,
            NFtype:NFTYPE_LITERAL|None=None,
            Halo:bool|None=None, Cyclic:bool|None=None, aperio:int|float|None=None,
            nperio:int|None=None, domain:Domain|None=None) -> xr.DataArray|None :
    '''
    Remove eastern most row
    '''
    push_stack ( f'lbc_del_cyclic (ptab, {domain=}, {cd_type=}, {psgn=}' )
...
def lbc_todom (ptab:xr.DataArray, dst_dom:Domain, src_dom:Domain|None=None, cd_type:CDTYPE_LITERAL|str='T',
    ...
    dst_dom   : Destination domain
    src_dom   : Source domain (if None, taken from ptab attributes)
    cd_type   : Grid specification : T, U, V or F
    psgn      : For change of sign for vector components
           (1 for scalars, -1 for vector components)
    See NEMO documentation for further details
...
@validate_types
def lbc_index (jj:int, ii:int, jpj:int, jpi:int, cd_type:CDTYPE_LITERAL|str='T',
               Iperio:bool|None=None, Jperio:bool|None=None, NFold:bool|None=None,
               NFtype:NFTYPE_LITERAL|None=None, Halo:bool|None=None,
               Cyclic:bool|None=None, aperio:int|float|None=None, nperio:int|None=None,
               domain:Domain|None=None) -> tuple[list[int]|int, list[int]|int] :
    '''
    ...
    zdom = Domain (jpi=jpi, jpj=jpj, Iperio=Iperio, Jperio=Jperio, NFold=NFold, NFtype=NFtype,
                        Halo=Halo, Cyclic=Cyclic,
                        aperio=aperio, nperio=nperio, domain=domain)
    jpj, jpi = zdom.jpj, zdom.jpi # type: ignore
...
@validate_types
def stack_yx (ptab, yxdim:str|None=None, Debug:bool=False) :
    '''
...
def find_ji (lat_data:xr.DataArray, lon_data:xr.DataArray, lat_grid:xr.DataArray, lon_grid:xr.DataArray,
    ...
    if ay is None :
        ay, _ = find_axis (lon_grid, 'y')
    if ax is None:
        ax, _ = find_axis (lon_grid, 'x')
...
def curl (tx:xr.DataArray, ty:xr.DataArray, e1u:xr.DataArray, e2v:xr.DataArray, e1f:xr.DataArray, e2f:xr.DataArray,
    ...
    zcurl = lbc_todom (zcurl, src_dom=zdom_ext, dst_dom=zdom, cd_type='F', psgn=1)

    pop_stack ( 'curl' )
    return zcurl

@validate_types
def div (ux, uy, e1t, e2t, e1v, e2u, Iperio:bool|None=None, Jperio:bool|None=None,
    NFold:bool|None=None, NFtype:Optional[NFTYPE_LITERAL]=None, Halo:bool|None=None,
    Cyclic:bool|None=None, aperio:int|float|None=None, nperio:int|None=None,
    domain:Domain|None=None, Debug:bool=False) -> xr.DataArray:
    '''
    ...
    ayt, _ = find_axis (e1t, 'y')
    axu, _ = find_axis (ux , 'x')
    ayu, _ = find_axis (ux , 'y')
    axv, _ = find_axis (uy , 'x')
    ayv, _ = find_axis (uy , 'y')
    ...
    zdiv = lbc_todom (zdiv, src_dom=zdom_ext, dst_dom=zdom, cd_type='T', psgn=1)
...
@validate_types
def en2geo (pte:xr.DataArray, ptn:xr.DataArray, glam:xr.DataArray, gphi:xr.DataArray) -> tuple[xr.DataArray, xr.DataArray, xr.DataArray] :
    '''
...
def angle_full (glamt:xr.DataArray, gphit:xr.DataArray, glamu:xr.DataArray, gphiu:xr.DataArray,
    ...
    zxnpt = 0. - 2.0 * np.cos (RAD*(zlamt)) * np.tan (np.pi/4.0 - np.deg2rad(zphit/2.0))
    zynpt = 0. - 2.0 * np.sin (RAD*zlamt) * np.tan (np.pi/4.0 - np.deg2rad(zphit/2.0))
    znnpt = zxnpt*zxnpt + zynpt*zynpt

    # north pole direction & modulous (at U-point)
    zxnpu = 0. - 2.0 * np.cos (RAD*zlamu) * np.tan (np.pi/4.0 - np.deg2rad(zphiu/2.0))
    zynpu = 0. - 2.0 * np.sin (RAD*zlamu) * np.tan (np.pi/4.0 - np.deg2rad(zphiu/2.0))
    znnpu = zxnpu*zxnpu + zynpu*zynpu

    # north pole direction & modulous (at V-point)
    zxnpv = 0. - 2.0 * np.cos (RAD*zlamv) * np.tan (np.pi/4.0 - np.deg2rad(zphiv/2.0))
    zynpv = 0. - 2.0 * np.sin (RAD*zlamv) * np.tan (np.pi/4.0 - np.deg2rad(zphiv/2.0))
    znnpv = zxnpv*zxnpv + zynpv*zynpv

    # north pole direction & modulous (at F-point)
    zxnpf = 0. - 2.0 * np.cos (RAD*zlamf) * np.tan (np.pi/4. - np.deg2rad(zphif/2.))
    zynpf = 0. - 2.0 * np.sin (RAD*zlamf) * np.tan (np.pi/4. - np.deg2rad(zphif/2.))
    ...
    zxvvt =  2.0 * np.cos (RAD*zlam) * np.tan (np.pi/4. - np.deg2rad(zphi/2.)) \
          -  2.0 * np.cos (RAD*zlan) * np.tan (np.pi/4. - np.deg2rad(zphh/2.))
    zyvvt =  2.0 * np.sin (RAD*zlam) * np.tan (np.pi/4. - np.deg2rad(zphi/2.)) \
          -  2.0 * np.sin (RAD*zlan) * np.tan (np.pi/4. - np.deg2rad(zphh/2.))
    ...
    zxffu =  2.0 * np.cos (RAD*zlam) * np.tan (np.pi/4. - np.deg2rad(zphi/2.)) \
          -  2.0 * np.cos (RAD*zlan) * np.tan (np.pi/4. - np.deg2rad(zphh/2.))
    zyffu =  2.0 * np.sin (RAD*zlam) * np.tan (np.pi/4. - np.deg2rad(zphi/2.)) \
          -  2.0 * np.sin (RAD*zlan) * np.tan (np.pi/4. - np.deg2rad(zphh/2.))
    ...
    zxffv =  2.0 * np.cos (RAD*zlam) * np.tan (np.pi/4. - np.deg2rad(zphi/2.)) \
          -  2.0 * np.cos (RAD*zlan) * np.tan (np.pi/4. - np.deg2rad(zphh/2.))
    zyffv =  2.0 * np.sin (RAD*zlam) * np.tan (np.pi/4. - np.deg2rad(zphi/2.)) \
          -  2.0 * np.sin (RAD*zlan) * np.tan (np.pi/4. - np.deg2rad(zphh/2.))
    ...
    zlan = zlamu
    zphh = zphiu
    zxuuf =  2. * np.cos ( RAD*zlam) * np.tan (np.pi/4. - np.deg2rad(zphi/2.)) \
          -  2. * np.cos ( RAD*zlan) * np.tan (np.pi/4. - np.deg2rad(zphh/2.))
    zyuuf =  2. * np.sin ( RAD*zlam) * np.tan (np.pi/4. - np.deg2rad(zphi/2.)) \
          -  2. * np.sin ( RAD*zlan) * np.tan (np.pi/4. - np.deg2rad(zphh/2.))
...
@validate_types
def angle (glam:xr.DataArray, gphi:xr.DataArray, cd_type:CDTYPE_LITERAL|str='T',
           Iperio:bool|None=None, Jperio:bool|None=None, NFold:bool|None=None,
           NFtype:NFTYPE_LITERAL|str|None=None, Halo:bool|None=None, Cyclic:bool|None=None,
           aperio:int|float|None=None, nperio:int|None=None, domain:Domain|None=None,
    ...
    #ax, _ = find_axis (glam, 'x')
    ay, _ = find_axis (glam, 'y')
    ...
    zxnp = 0. - 2.0 * np.cos (RAD*zlam) * np.tan (np.pi/4.0 - np.deg2rad(zphi/2.0))
    zynp = 0. - 2.0 * np.sin (RAD*zlam) * np.tan (np.pi/4.0 - np.deg2rad(zphi/2.0))
    znnp = zxnp*zxnp + zynp*zynp

    # j-direction: segment direction (around point)
    zlan_n = zlam.roll ({ay:-1}) # glam [jj+1, ji]
    zphh_n = zphi.roll ({ay:-1}) # gphi [jj+1, ji]
    zlan_s = zlam.roll ({ay: 1}) # glam [jj-1, ji]
    zphh_s = zphi.roll ({ay: 1}) # gphi [jj-1, ji]
    ...
    gsin = lbc_todom (gsin, src_dom=zdom_ext, dst_dom=zdom, cd_type=cd_type, psgn=-1.)
    gcos = lbc_todom (gcos, src_dom=zdom_ext, dst_dom=zdom, cd_type=cd_type, psgn=-1.)
...
def rot_en2ij ( u_e:xr.DataArray, v_n:xr.DataArray, gsin:xr.DataArray, gcos:xr.DataArray,
                cd_type:CDTYPE_LITERAL|str='T', Iperio:bool|None=None, Jperio:bool|None=None,
                NFold:bool|None=None, NFtype:NFTYPE_LITERAL|None=None, Halo:bool|None=None,
                Cyclic:bool|None=None, aperio:int|float|None=None, nperio:int|None=None,
...
def rot_ij2en ( u_i:xr.DataArray, v_j:xr.DataArray, gsin:xr.DataArray, gcos:xr.DataArray,
               cd_type:CDTYPE_LITERAL|str|None, Iperio:bool|None=None, Jperio:bool|None=None,
               NFold:bool|None=None, NFtype:NFTYPE_LITERAL|str|None=None, Halo:bool|None=None,
               Cyclic:bool|None=None, aperio:int|float|None=None, domain:Domain|None=None,
               ) -> tuple[xr.DataArray, xr.DataArray] :
    '''
    ...
    zdom   = Domain (ptab=u_i, Iperio=Iperio, Jperio=Jperio, NFold=NFold, NFtype=NFtype,
                    Halo=Halo, Cyclic=Cyclic, aperio=aperio, domain=domain)
...
def u2t (utab:xr.DataArray, psgn:int|float=-1, zdim:str|None=None, action:str='ave',
         Iperio:bool|None=None, Jperio:bool|None=None, NFold:bool|None=None,
         NFtype:Optional[NFTYPE_LITERAL]=None, Halo:bool|None=None, Cyclic:bool|None=None,
         aperio:int|float|None=None, nperio:int|None=None, domain:Domain|None=None, Debug=False,
         ) -> xr.DataArray :
    '''
    ...
    if ax :
        if action == 'ave' :
            ttab = 0.5 *      (utab_0 + utab_0.roll ({ax:1}))
        elif action == 'min' :
            ttab = np.minimum (utab_0 , utab_0.roll ({ax:1}))
        elif action == 'max' :
            ttab = np.maximum (utab_0 , utab_0.roll ({ax:1}))
        elif action == 'mult':
            ttab =             utab_0 * utab_0.roll ({ax:1})
        else :
            raise ValueError ( f'u2t: Unknown action={action}' )
        ttab = lbc_todom (ttab  , src_dom=zdom_ext, dst_dom=zdom, cd_type='T', psgn=psgn)
    else :
...
def v2t (vtab:xr.DataArray, psgn:int|float=-1, zdim:str|None=None, action:str='ave',
    ...
    if ay :
        if action == 'ave'  :
            ttab = 0.5 *      (vtab_0 + vtab_0.roll ({ay:1}))
        elif action == 'min'  :
            ttab = np.minimum (vtab_0 , vtab_0.roll ({ay:1}))
        elif action == 'max'  :
            ttab = np.maximum (vtab_0 , vtab_0.roll ({ay:1}))
        elif action == 'mult' :
            ttab =             vtab_0 * vtab_0.roll ({ay:1})
        else :
            raise ValueError ( f'v2t: Unknown action={action}' )
        ttab = lbc_todom (ttab  , src_dom=zdom_ext, dst_dom=zdom, cd_type='T', psgn=psgn)
    else :
...
def t2u (ttab:xr.DataArray, psgn:int|float=1, zdim:str|None=None, action:str|None='ave',
    ...
    az, _  = find_axis (ttab_0, 'z')
    if ix :
        if action == 'ave'  :
            utab = 0.5 *      (ttab_0 + ttab_0.roll ({ax:-1}))
        elif action == 'min'  :
            utab = np.minimum (ttab_0 , ttab_0.roll ({ax:-1}))
        elif action == 'max'  :
            utab = np.maximum (ttab_0 , ttab_0.roll ({ax:-1}))
        elif action == 'mult' :
            utab =             ttab_0 * ttab_0.roll ({ax:-1})
        else :
            raise ValueError ( f't2u: Unknown action={action}' )
        utab = lbc_todom (utab  , src_dom=zdom_ext, dst_dom=zdom, cd_type='U', psgn=psgn)
    else :
...
def t2v (ttab:xr.DataArray, psgn:int|float=1, zdim=None, action='ave',
    ...
    if jy :
        if action == 'ave'  :
            vtab = 0.5 *      (ttab_0 + ttab_0.roll ({ay:-1}))
        elif action == 'min'  :
            vtab = np.minimum (ttab_0 , ttab_0.roll ({ay:-1}))
        elif action == 'max'  :
            vtab = np.maximum (ttab_0 , ttab_0.roll ({ay:-1}))
        elif action == 'mult' :
            vtab =             ttab_0 * ttab_0.roll ({ay:-1})
        else :
            raise ValueError ( f't2v: Unknown action={action}' )
        vtab = lbc_todom (vtab  , src_dom=zdom_ext, dst_dom=zdom, cd_type='V', psgn=psgn)
    else :
...
def v2f (vtab:xr.DataArray, psgn:int|float=-1, zdim:str|None=None, action:str='ave',
    ...
    if ix :
        if action == 'ave'  :
            ftab = 0.5 *      (vtab_0 + vtab_0.roll ({ax:-1}))
        elif action == 'min'  :
            ftab = np.minimum (vtab_0 , vtab_0.roll ({ax:-1}))
        elif action == 'max'  :
            ftab = np.maximum (vtab_0 , vtab_0.roll ({ax:-1}))
        elif action == 'mult' :
            ftab =             vtab_0 * vtab_0.roll ({ax:-1})
        else :
            raise ValueError ( f'v2f: Unknown action={action}' )
        ftab = lbc_todom (ftab  , src_dom=zdom_ext, dst_dom=zdom, cd_type='F', psgn=psgn)
    else :
...
def u2f (utab:xr.DataArray, psgn:int|float=-1, zdim:str|None=None, action:str='ave',
         Iperio:bool|None=None, Jperio:bool|None=None, NFold:bool|None=None,
         NFtype:NFTYPE_LITERAL|None=None, Halo:bool|None=None, Cyclic:bool|None=None,
         aperio:int|float|None=None, nperio:int|None=None, domain:Domain|None=None,
         ) -> xr.DataArray :
    '''
    ...
    if jy :
        if action == 'ave'  :
            ftab = 0.5 *      (utab_0 + utab_0.roll ({ay:-1}))
        elif action == 'min'  :
            ftab = np.minimum (utab_0 , utab_0.roll ({ay:-1}))
        elif action == 'max'  :
            ftab = np.maximum (utab_0 , utab_0.roll ({ay:-1}))
        elif action == 'mult' :
            ftab =             utab_0 * utab_0.roll ({ay:-1})
        else :
            raise ValueError ( f'u2f: Unknown action={action}' )
        ftab = lbc_todom (ftab  , src_dom=zdom_ext, dst_dom=zdom, cd_type='F', psgn=psgn)
    else :
...
@validate_types
def t2f (ttab:xr.DataArray, psgn:int|float=1, zdim:str|None=None, action:str='ave',
         Iperio:bool|None=None, Jperio:bool|None=None, NFold:bool|None=None,
         NFtype:NFTYPE_LITERAL|str|None=None, Halo:bool|None=None, Cyclic:bool|None=None,
         aperio:int|float|None=None, nperio:int|None=None, domain:Domain|None=None,
         ) -> xr.DataArray :
...
def f2u (ftab:xr.DataArray, psgn:int|float=1, zdim:str|None=None, action:str='ave',
    ...
    az, _  = find_axis (ftab_0, 'z')
    if jy :
        if action == 'ave'  :
            utab = 0.5 *      (ftab_0 + ftab_0.roll ({ay:-1}))
        elif action == 'min'  :
            utab = np.minimum (ftab_0 , ftab_0.roll ({ay:-1}))
        elif action == 'max'  :
            utab = np.maximum (ftab_0 , ftab_0.roll ({ay:-1}))
        elif action == 'mult' :
            utab =             ftab_0 * ftab_0.roll ({ay:-1})
        else :
            raise ValueError ( f'f2u: Unknown action={action}' )
        utab = lbc_todom (utab  , src_dom=zdom_ext, dst_dom=zdom, cd_type='U', psgn=psgn)
    else :
...
def f2v (ftab:xr.DataArray, psgn:int|float=1, zdim:str|None=None, action:str='ave',
    ...
    if ix :
        if action == 'ave'  :
            vtab = 0.5 *      (ftab_0 + ftab_0.roll ({ax:-1}))
        elif action == 'min'  :
            vtab = np.minimum (ftab_0 , ftab_0.roll ({ax:-1}))
        elif action == 'max'  :
            vtab = np.maximum (ftab_0 , ftab_0.roll ({ax:-1}))
        elif action == 'mult' :
            vtab =             ftab_0 * ftab_0.roll ({ax:-1})
        else :
            raise ValueError ( f'Unknown action {action} in f2v' )
        vtab = lbc_todom (vtab  , src_dom=zdom_ext, dst_dom=zdom, cd_type='V', psgn=psgn)
    else :
...
def fill (ptab:xr.DataArray, cd_type:CDTYPE_LITERAL|str='T', npass:int=1, sval=np.nan,
    ...
    for _ in range (npass) :
        ...
        zcount = 1./6. * ( zmask \
          + zmask.roll({ax:1}) + zmask.roll({ax:-1}) + zmask.roll({ay:1}) + zmask.roll({ay:-1}) \
          + 0.5 * ( \
                + zmask.roll({ay: 1, ax: 1}) + zmask.roll({ay:-1, ax: 1}) \
                + zmask.roll({ay: 1, ax:-1}) + zmask.roll({ay:-1, ax:-1}) ) )

        znew = 1./6. * ( ztab0 \
          + ztab0.roll({ax:1}) + ztab0.roll({ax:-1}) + ztab0.roll({ay:1}) + ztab0.roll({ay:-1}) \
          + 0.5 * ( \
                + ztab0.roll({ay: 1, ax: 1}) + ztab0.roll({ay:-1, ax: 1}) \
                + ztab0.roll({ay: 1, ax:-1}) + ztab0.roll({ay:-1, ax:-1}) ) )
    ...
    if do_perio :
        ztab = lbc_todom (ztab, src_dom=zdom_ext, dst_dom=ldom)
...
def zmsf_index (zmsf:xr.DataArray, bname:Literal['nadw', 'aabw', 'npdw', 'deacon']='nadw',
    ...
    if bname == "nadw"   :
        ...
        raise ValueError ( f'zmsf_index : unknown bname={bname}' )
    ...
    return index_ocean # type: ignore
...
@validate_types
def namelist_read (ref:str|None=None, cfg:str|None=None, out:str='dict',
                    flat:bool=False, Debug:bool=False
                    ) -> dict|xr.DataArray|xr.Dataset|None :
    '''
    ...
    if f90nml is not None :
        ...
        if out == 'dict' :
            ...
            raise ValueError ( f'namelist_read : unknown out={out}' )
...
@validate_types
def fill_closed_seas (imask:xr.DataArray, cd_type:CDTYPE_LITERAL|str='T',
                      Iperio:bool|None=None, Jperio:bool|None=None,
